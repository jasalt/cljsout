...
(deftype Events [ch mult head closed disposers]
  ...
  IFn
         ...
         (-invoke [stream msg] (do (go (>! ch (box msg))) stream))
        ...
        (let [[_ port] (a/alts! [stop (a/timeout interval)])]
          (when (not= port stop)
            (let [val @ref]
                     ...
                     (when-not (undefined? val) (>! out (box val)))
...
;;;;;;;;;;;; This file autogenerated from src/cljx/reagi/core.cljx
